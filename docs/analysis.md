...
| :--- | :--- | :---: | :---: | :---: | :---: |
| **Sequential (CPU Baseline)** | Waktu (s)<br>Speedup<br>GFLOPS | 0.0084 s<br>1.00x<br>3.99 | 0.0656 s<br>1.00x<br>4.09 | 2.4611 s<br>1.00x<br>0.87 | 23.7939 s<br>1.00x<br>0.72 |
| **OpenMP (6 Threads CPU)** | Waktu (s)<br>Speedup<br>GFLOPS | 0.0025 s<br>3.41x<br>13.59 | 0.0128 s<br>5.13x<br>21.00 | 0.3897 s<br>6.32x<br>5.51 | 3.1268 s<br>7.61x<br>5.49 |
| **OpenCL (GPU Tiled)** | Waktu (s)<br>Speedup<br>GFLOPS | 0.0911 s<br>0.09x<br>0.37 | 0.0867 s<br>0.76x<br>3.10 | 0.1012 s<br>24.31x<br>21.22 | 0.1517 s<br>156.87x<br>113.26 |
...
| :---: | :---: | :---: | :---: | :---: | :---: |
| **N = 256** | 0.00018 s (0.19%) | 0.00012 s (0.13%) | 0.00005 s (0.05%) | 0.09075 s (99.63%) | 0.09109 s |
| **N = 512** | 0.00041 s (0.48%) | 0.00077 s (0.89%) | 0.00016 s (0.18%) | 0.08533 s (98.46%) | 0.08667 s |
| **N = 1024** | 0.00101 s (1.00%) | 0.00647 s (6.39%) | 0.00041 s (0.41%) | 0.09333 s (92.20%) | 0.10122 s |
| **N = 2048** | 0.00378 s (2.49%) | 0.05161 s (34.02%) | 0.00141 s (0.93%) | 0.09488 s (62.55%) | 0.15168 s |

**Analisis Overhead:**
Data di atas membuktikan secara konkrit bahwa pada ukuran matriks kecil ($N \le 512$), rendahnya performa GPU murni disebabkan oleh dominasi **Launch Overhead & JIT** yang mencapai $>98\%$. Waktu pemrosesan aktif ($T_{H2D} + T_{kernel} + T_{D2H}$) sebenarnya sangat kecil (hanya 0.35 ms pada $N=256$). Pada $N=2048$, porsi pengerjaan kernel murni mulai meningkat secara signifikan menjadi $34.02\%$ dari total waktu, memvalidasi efisiensi GPU seiring meningkatnya ukuran masalah.
...
* **Hukum Skala Kinerja (Scaling Law):** Ketika ukuran $N$ berlipat ganda dari 1024 ke 2048, jumlah operasi aritmatika teoritis meningkat sebanyak $2^3 = 8$ kali.
  * Pada **CPU Sequential**, waktu naik dari 2.4611s ke 23.7939s ($9.67\times$). Kenaikan ini konsisten secara konseptual dengan kompleksitas $\mathcal{O}(N^3)$, namun jauh lebih tinggi dari $8\times$ akibat meningkatnya laju *cache miss* pada batas fisik memori CPU.
  * Pada **OpenCL GPU**, execution time kernel murni ($T_{kernel}$) naik dari 0.00647s ke 0.05161s ($7.98\times$). Angka ini sangat mendekati $8\times$ karena GPU semakin mendekati utilisasi hardware yang optimal seiring besarnya workload, membuktikan efisiensi data reuse (tiling) pada aritmetika berintensitas tinggi.
...
* **Compute-bound Region ($N \ge 1024$):** Pada ukuran besar, Intensitas Aritmatika meningkat tajam (mencapai 341.3 FLOP/byte pada $N=2048$). Sistem melintasi titik belok (*knee point*) Roofline Model ke dalam wilayah komputasi murni. GPU NVIDIA RTX 4050 berhasil memanfaatkan ribuan *compute cores* secara optimal, melesatkan throughput hingga **113.26 GFLOPS** murni.
